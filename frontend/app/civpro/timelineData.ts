...
export const stages: Stage[] = [
  ...
  },
  {
    ...
    concepts: [
      ...
      { name: 'Dispositive Motions — Strategic Sequence', description: 'Before/during pleadings: Rule 41(a) voluntary dismissal (1st = without prejudice, 2nd = with prejudice). After discovery closes: Rule 56 Summary Judgment — "Facts undisputed, no trial needed." During trial after opposing evidence: Rule 50(a) JMOL — "No rational jury could find for them. End it now." MUST FILE THIS or lose the right to file 50(b) after verdict! After jury verdict: Rule 50(b) Renewed JMOL (28 days, must mirror prior 50(a)) AND/OR Rule 59 New Trial — "Something went wrong, redo the trial."' },
    ],
    keyCases: [
  ...
  },
  {
    ...
    rules: [
      ...
      { rule: 'Rule 50(c)', slug: 'rule-50', description: 'Combined JMOL + new trial motions — conditional ruling on new trial', timing: 'Expressly allows filing JMOL and new trial motions together. If JMOL is granted, court must CONDITIONALLY rule on new trial motion too. This prevents needing a remand if the appellate court reverses JMOL — the conditional new trial ruling is already in place. Safe practice: always move for JMOL before verdict, then after adverse verdict renew JMOL AND move for new trial in the alternative.' },
      ...
      { rule: 'Rule 59(a)(1)', slug: 'rule-59', description: 'Partial new trial — retry some issues while preserving others', timing: 'Allows new trial "on all or SOME of the issues." The issue to be retried must be "so distinct and separable from the others that a trial of it alone may be had without injustice." Example: if damages are interlinked with liability, must retry both. If liability was bifurcated and decided separately, can retry damages only.' },
      { rule: 'Rule 61', slug: null, description: 'Harmless error — no new trial unless error affected substantial rights', timing: 'Applies at the new trial stage. A process error (bad evidentiary ruling, improper argument, juror misconduct, instruction error) does NOT warrant a new trial unless it "affected any party\'s substantial rights." Standards: error "highly probably," "to a substantial degree," or "more probably than not" tainted the verdict. Cumulative evidence or errors with no real impact on the verdict = harmless.' },
      { rule: 'FRE 606(b)', slug: null, description: 'No-impeachment rule — jurors cannot testify about deliberations', timing: 'General rule: jurors CANNOT testify about deliberations or their mental processes. Three exceptions — jurors MAY testify about: (1) extraneous prejudicial information improperly brought to jury\'s attention, (2) outside influence improperly brought to bear on a juror, (3) error in entering the verdict form. Court applies OBJECTIVE test: would the information affect a typical juror? Cannot inquire into subjective effect on particular jurors. Additional exception: 6th Amendment requires an exception when a juror makes a clear statement relying on racial stereotypes or animus.' },
  ...
  },
  {
    ...
    rules: [
      ...
      { rule: 'Rule 12(b)(6)', slug: 'rule-12', description: 'Dismissal with prejudice for failure to state a claim = on the merits (preclusive)', timing: 'A 12(b)(6) dismissal WITH prejudice counts as a final judgment on the merits for claim preclusion purposes. However, dismissals for lack of SMJ (12(b)(1)), lack of PJ (12(b)(2)), improper venue (12(b)(3)), or failure to join indispensable party (12(b)(7)) are NOT on the merits and NOT preclusive.' },
    ],
    concepts: [
      { name: 'Claim Preclusion (Res Judicata) — Three Elements', description: 'A final judgment on the merits bars suing again on the same claim. Three elements: (1) Same claim as previously litigated, (2) Valid, final judgment on the merits, (3) Same parties or those in privity. Bars claims that WERE or COULD HAVE BEEN litigated in the first action. All theories of recovery arising from the same operative facts must be brought in a single lawsuit or risk preclusion.' },
      { name: 'Defining "Same Claim" — Transactional Test (Majority Rule)', description: 'Three competing tests: (1) Transactional test (broadest, modern majority, Restatement 2d § 24): claims are "same" if arising from a single group of operative facts, regardless of different legal theories, different relief sought, or different evidence needed. (2) Same evidence test (narrower): claims are same only if evidence needed for second suit would sustain the first. (3) Primary rights test (narrowest): separate claim for each type of injury (bodily injury, property damage, lost profits = 3 claims). Transactional test best serves efficiency, fairness to defendants, and consistency — but poses greatest risk of inadvertent forfeiture for plaintiffs.' },
      { name: 'Valid, Final Judgment "On the Merits"', description: 'Judgment must be valid (court had PJ and SMJ, proper notice), final (ends litigation on merits), and on the merits (resolves substance, not just procedure). ON the merits: judgment after trial, summary judgment, dismissal WITH prejudice under 12(b)(6). NOT on the merits: dismissal for lack of SMJ (12(b)(1)), lack of PJ (12(b)(2)), improper venue (12(b)(3)), failure to join party (12(b)(7)), dismissal WITHOUT prejudice. An invalid judgment (no jurisdiction, no notice) has NO preclusive effect.' },
      { name: 'Non-Party Preclusion & Privity', description: 'Claim preclusion typically only binds parties to original lawsuit and those in "privity" — a close legal relationship (successor in interest, predecessor, beneficiary/trustee). A party\'s representation of a non-party\'s interest in prior litigation is usually insufficient to bind the non-party absent a specific pre-existing legal relationship.' },
      { name: 'Exceptions to Claim Preclusion', description: 'Even if all three elements are met, claim preclusion may not apply if: (1) first court lacked jurisdiction, (2) insufficient notice or opportunity to be heard, (3) first action was fundamentally unfair, (4) parties expressly agreed first action would not be preclusive, (5) second action asserts claims first court expressly reserved for later adjudication.' },
