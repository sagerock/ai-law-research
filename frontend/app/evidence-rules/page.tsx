...
export default function EvidenceRulesPage() {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/v1/legal-texts/fre`)
      .then(res => res.json())
      .then(data => {
        setItems(data.items || [])
        setLoading(false)
  ...
  const doSearch = useCallback((q: string) => {
    ...
    setSearching(true)
    fetch(`${API_URL}/api/v1/legal-texts/search?q=${encodeURIComponent(trimmed)}&doc_id=fre&limit=30`)
      .then(res => res.json())
      .then(data => {
      ...
      <section className="py-8 px-4">
        <div className="container mx-auto max-w-3xl">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="text-3xl font-bold text-stone-900">Federal Rules of Evidence</h2>
          </div>
          <p className="text-stone-600 mb-6">Browse and search all {items.length || '...'} rules.</p>
          ...
          {!showSearch && (
            loading ? (
              <p className="text-stone-500">Loading rules...</p>
            ) : (
              <div className="space-y-1">
                {items.map(item => (
                  ...
                  >
                    <span className="text-sm font-mono text-sage-600 w-12 flex-shrink-0 text-right">
                      {item.number}
                    </span>
