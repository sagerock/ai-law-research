...
interface MSJCaseInfoProps {
  caseInfo: CaseInfoType
  onChange: (info: CaseInfoType) => void
}

export default function MSJCaseInfo({ caseInfo, onChange }: MSJCaseInfoProps) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => updateField('representing_side', e.target.value)}
            className="w-full px-3 py-2 border border-stone-200 rounded-lg text-sm
                       focus:outline-none focus:ring-2 focus:ring-sage-500 focus:border-transparent"
          >
            <option value="">Select side...</option>
            <option value="plaintiff">Plaintiff (Movant)</option>
            <option value="defendant">Defendant (Movant)</option>
          </select>
        </div>
